# -*- coding: utf-8 -*-
"""
单季度营业收入同比增长率(Sales YoY, single quarter)因子：周频 Rank IC 回测。

营业收入指标为：
（financial_data.metric_code = B001101000）：

- 单季度拆解：Q1=一季报累计；Q2=半年报−一季报；Q3=三季报−半年报；Q4=年报−三季报。
- 单季度同比增长率 Qn_yoy =（本年 Qn−去年同季 Qn）/ |去年同季 Qn|。
...
- ROE 筛选：按点时可得（固定披露截止日）取 profitability_data 的
  净资产收益率(ROE)TTM（F050504C，小数形式），仅保留 ROE TTM > 阈值
  （默认 10%，--roe-min 可调，--no-roe-filter 可关闭）的 (股票, 信号日) 记录；
  无 ROE 数据的记录视为不满足条件，一并剔除。

运行：  D:/Anaconda/python.exe 财务因子/Sales_YoY_Q1_factor.py
"""
...
OUTPUT_DIR = PROJECT_DIR / "sales_yoy_q1_weekly_output"
...
REVENUE_METRIC_CODE = "B001101000"   # 营业收入
...
ROE_FILTER_ENABLED = True       # False 则不做 ROE 筛选
...
def load_financial_revenue(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    """读取营业收入长表；多取两年历史用于单季度同比（同比需去年同季，同季需再上一季累计）。"""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"sales_yoy_q1_{REVENUE_METRIC_CODE}_{start_date}_{end_date}.parquet"
    ...
    FROM financial_data
    WHERE metric_code = '{REVENUE_METRIC_CODE}'
...
def load_roe_ttm(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"sales_yoy_q1_roe_ttm_{ROE_TTM_FIELD}_{start_date}_{end_date}.parquet"
    if cache.exists() and not refresh:
        return pd.read_parquet(cache)
...
def load_market(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    """读取回测所需的行情列。"""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"sales_yoy_q1_market_{start_date}_{end_date}.parquet"
    if cache.exists() and not refresh:
        return pd.read_parquet(cache)
...
def prepare_sales_yoy_single_quarter(
    raw_financial: pd.DataFrame,
    availability: str = "deadline",
) -> pd.DataFrame:
    """从 financial_data 计算单季度营业收入同比增长率。
    ...
    增长率使用（本年单季−去年同季单季）/ |去年同季单季|；分母为 0 的记录剔除。
    ...
    fin["revenue_cum"] = pd.to_numeric(fin["metric_value"], errors="coerce")
    fin = fin.loc[fin["report_date"].dt.strftime("%m-%d").isin(STANDARD_PERIOD_MMDD)].copy()
    fin = fin.dropna(subset=["stock_code", "report_date", "revenue_cum"])
    ...
    # 单季度营业收入 = 本期累计 − 上一季度累计（Q1 直接取累计）。
    fin["prev_q_end"] = (
        (fin["report_date"].dt.to_period("Q") - 1).dt.to_timestamp(how="end").dt.normalize()
    )
    prev_cum = fin[["stock_code", "report_date", "revenue_cum"]].rename(
        columns={"report_date": "prev_q_end", "revenue_cum": "prev_cum"}
    )
    fin = fin.merge(prev_cum, on=["stock_code", "prev_q_end"], how="left")
    is_q1 = fin["report_date"].dt.quarter.eq(1)
    fin["revenue_sq"] = fin["revenue_cum"].where(is_q1, fin["revenue_cum"] - fin["prev_cum"])

    # 单季度同比增长率 =（本年单季−去年同季单季）/ |去年同季单季|。
    fin["prior_year_end"] = fin["report_date"] - pd.DateOffset(years=1)
    prev_year = fin[["stock_code", "report_date", "revenue_sq"]].rename(
        columns={"report_date": "prior_year_end", "revenue_sq": "prev_year_revenue_sq"}
    )
    fin = fin.merge(prev_year, on=["stock_code", "prior_year_end"], how="left")

    valid = (
        fin["revenue_sq"].notna()
        & fin["prev_year_revenue_sq"].notna()
        & fin["prev_year_revenue_sq"].ne(0)
    )
    fin["sales_yoy_sq"] = np.nan
    fin.loc[valid, "sales_yoy_sq"] = (
        (fin.loc[valid, "revenue_sq"] - fin.loc[valid, "prev_year_revenue_sq"])
        / fin.loc[valid, "prev_year_revenue_sq"].abs()
    )
    factor = fin.loc[valid, [
        "stock_code", "report_date", "available_date",
        "revenue_cum", "revenue_sq", "prev_year_revenue_sq", "sales_yoy_sq",
    ]].copy()
...
def attach_factor_pit(universe_at_signal: pd.DataFrame, factor: pd.DataFrame) -> pd.DataFrame:
    ...
    # 双键排序：同一 available_date 出现多条时（如4/30年报+一季报同日可得），
    # 保证 merge_asof 平局取 report_date 最新的一条，结果与排序算法稳定性无关。
    right = factor.sort_values(["available_date", "report_date"])[
        ["stock_code", "available_date", "report_date", "sales_yoy_sq"]
    ].copy()
    ...
    return merged.dropna(subset=["sales_yoy_sq"]).copy()
...
def attach_roe_pit(signal_factor: pd.DataFrame, roe: pd.DataFrame) -> pd.DataFrame:
    ...
    平局必须确定性地取最新报告期（一季报），不能依赖 sort_values 的不稳定快排。
    """
...
def process_cross_section(signal_factor: pd.DataFrame, group_num: int) -> pd.DataFrame:
    ...
    df = signal_factor.dropna(subset=["sales_yoy_sq"]).copy()
    g = df.groupby("signal_date")["sales_yoy_sq"]
    mean = g.transform("mean")
    std = g.transform(lambda s: s.std(ddof=0))
    lower, upper = mean - 3.0 * std, mean + 3.0 * std
    df = df.loc[(df["sales_yoy_sq"] >= lower) & (df["sales_yoy_sq"] <= upper)].copy()

    g2 = df.groupby("signal_date")["sales_yoy_sq"]
    mean2 = g2.transform("mean")
    std2 = g2.transform(lambda s: s.std(ddof=0))
    df["zscore"] = np.where(std2.gt(0), (df["sales_yoy_sq"] - mean2) / std2, np.nan)
...
def backtest_variant(factor: pd.DataFrame, universe: pd.DataFrame, fwd: pd.DataFrame,
    ...
    """给定单季度营业收入同比增长率因子，做周频 10 组多空 + RankIC + 绩效，落盘到 out_dir。
    ...
    factor.to_csv(out_dir / "01_sales_yoy_q1_weekly_factor.csv", index=False, encoding="utf-8-sig")
    graded[["signal_date", "stock_code", "sales_yoy_sq", "roe_ttm", "zscore", "rank_desc",
...
def run(start_date: str, end_date: str, group_num: int, return_column: str,
    ...
    print("[1/4] 读取财务数据（营业收入）...")
    raw_fin = load_financial_revenue(start_date, end_date, refresh)
    ...
    factor = prepare_sales_yoy_single_quarter(raw_fin, availability=availability)
    variant_name = OUTPUT_VARIANT_NAME
    if roe is not None:
        variant_name = f"{OUTPUT_VARIANT_NAME}_roe_gt_{roe_min_pct:g}"
    out_dir = OUTPUT_DIR / variant_name
    ...
    print(f"      [{variant_name}] 因子记录 {summary['factor_rows']}，"
          f"股票 {summary['n_stocks']} → {out_dir}")

    print("\n================= 单季度营业收入同比增长率因子(Sales_YoY_Q1)周频回测结果摘要 =================")
    pf = summary["perf_full"]
    print(f"\n【{variant_name}】{OUTPUT_VARIANT_DESCRIPTION}"
...
def parse_args():
    p = argparse.ArgumentParser(description="单季度营业收入同比增长率因子(Sales_YoY_Q1)周频回测（文档口径）")
