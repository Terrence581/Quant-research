# -*- coding: utf-8 -*-
"""
单季度营收同比因子(SalesYOY Q1)复现——图表脚本。
读取 Sales_YoY_Q1_factor.py 的输出，绘制对应研报"原始因子"的图：
- 多空组合净值（对应图18左 / 图26左的"原始因子"曲线）
- 分组净值曲线 group_1..group_10（对应图19左）
- 周频 Rank IC 柱状 + 累计 Rank IC（沿用图20 原始因子的展示形式）

输出目录与 ROE 筛选口径自动跟随 Sales_YoY_Q1_factor.py 参数区
（ROE_FILTER_ENABLED / ROE_MIN_PCT），无需手动同步。

运行：  D:/Anaconda/python.exe 财务因子/Sales_YoY_Q1_factor_analysis.py
"""
...
from Sales_YoY_Q1_factor import (  # noqa: E402
...
def plot_long_short_nav(ls: pd.DataFrame, path: Path) -> None:
    ...
    ax.plot(ls["signal_date"], ls["long_short_nav"], color="#c0392b", lw=1.6,
            label="原始因子 多空净值")
    ax.set_title(f"单季度营收同比因子(SalesYOY Q1) 原始因子 多空组合净值（周频）{FILTER_TAG}")
    ax.set_ylabel("净值")
...
def plot_group_nav(groups: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"SalesYOY Q1 原始因子 分组净值{FILTER_TAG}（group_1=因子最高, group_10=因子最低）")
    ax.set_ylabel("净值")
...
def plot_rank_ic(ic: pd.DataFrame, path: Path) -> None:
    ...
    ax1.set_title(f"SalesYOY Q1 原始因子 周频 Rank IC（mean {mean_ic*100:.2f}%）{FILTER_TAG}")
    ax1.grid(alpha=0.3)
...
def plot_quantile_sort_bar(groups: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"单季度营收同比排序检验{FILTER_TAG}")
    ax.set_xlabel("单季度营收同比因子分组")
...
def plot_sort_test_table(groups: pd.DataFrame, ls: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"单季度营收同比排序检验结果{FILTER_TAG}", pad=20)
    fig.tight_layout()
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def plot_high_low_nav(ls: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle(f"单季度营收同比因子 High-Low 周频复利净值走势{FILTER_TAG}", y=0.985, fontsize=13)
...
def plot_ic(ic: pd.DataFrame, path: Path) -> None:
    ...
    mean_ic = ic["ic"].mean()
    ax1.set_title(f"单季度营收同比因子 IC（mean {mean_ic*100:.2f}%）{FILTER_TAG}")
    ax1.set_xlabel("时间")
    ax1.grid(alpha=0.3)
    ax1.xaxis.set_major_locator(mdates.YearLocator())
    ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
