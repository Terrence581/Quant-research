# -*- coding: utf-8 -*-
"""
单季度营业收入环比增长率(Revenue QoQ, single quarter)因子：周频 Rank IC 回测。

参照 财务因子/Sales_YoY_Q1_factor.py 的框架搭建，标的指标为
营业收入（financial_data.metric_code = B001101000），时序规则复用本目录
《基本面因子回测时序规则.md》的约定（对营收成立的拆解逻辑
同样适用于营业收入，只是营业收入可能为负，见下文口径说明）：

- 单季度拆解：Q1=一季报累计；Q2=半年报−一季报；Q3=三季报−半年报；Q4=年报−三季报。
- 单季度环比增长率 Qn_qoq =（本季单季−上季单季）/ |上季单季|（分母取绝对值，
  使增长方向在上季基数为负时仍保持一致）。
...
运行：  D:/Anaconda/python.exe 财务因子/Revenue_QoQ_weekly_factor.py
"""
...
OUTPUT_DIR = PROJECT_DIR / "revenue_qoq_weekly_output"
...
def load_financial_revenue(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    """读取营业收入长表；多取两年历史，保证起始期 Q1 能拆出上年 Q4 单季值。"""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"revenue_{REVENUE_METRIC_CODE}_{start_date}_{end_date}.parquet"
...
def load_market(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    """读取回测所需的行情列。"""
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"revenue_market_{start_date}_{end_date}.parquet"
    if cache.exists() and not refresh:
        return pd.read_parquet(cache)
...
def load_roe_ttm(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"revenue_qoq_roe_ttm_{ROE_TTM_FIELD}_{start_date}_{end_date}.parquet"
    if cache.exists() and not refresh:
        return pd.read_parquet(cache)
...
def prepare_revenue_qoq_single_quarter(
    raw_financial: pd.DataFrame,
    availability: str = "deadline",
) -> pd.DataFrame:
    """从 financial_data 计算单季度营业收入环比增长率。
    ...
    营业收入在极端情况下可能为负，增长率使用
    （本季单季−上季单季）/ |上季单季|；分母为 0 的记录剔除。
    ...
    # 单季度环比增长率 =（本季单季−上季单季）/ |上季单季|。Q1 的上季是上年 Q4。
    prev_quarter = fin[["stock_code", "report_date", "revenue_sq"]].rename(
        columns={"report_date": "prev_q_end", "revenue_sq": "prev_quarter_sq"}
    )
    fin = fin.merge(prev_quarter, on=["stock_code", "prev_q_end"], how="left")

    valid = (
        fin["revenue_sq"].notna()
        & fin["prev_quarter_sq"].notna()
        & fin["prev_quarter_sq"].ne(0)
    )
    fin["revenue_qoq_sq"] = np.nan
    fin.loc[valid, "revenue_qoq_sq"] = (
        (fin.loc[valid, "revenue_sq"] - fin.loc[valid, "prev_quarter_sq"])
        / fin.loc[valid, "prev_quarter_sq"].abs()
    )
    factor = fin.loc[valid, [
        "stock_code", "report_date", "available_date",
        "revenue_cum", "revenue_sq", "prev_quarter_sq", "revenue_qoq_sq",
    ]].copy()
    # 单季度拆解需相邻季度，故在算完环比后仅保留 next_q1_fill 使用的 Q1、Q2、Q3。
...
def attach_factor_pit(universe_at_signal: pd.DataFrame, factor: pd.DataFrame) -> pd.DataFrame:
    """对每个 (股票, 信号日) 取 available_date <= 信号日 的最新单季度环比增长率因子。"""
    ...
    right = factor.sort_values(["available_date", "report_date"])[
        ["stock_code", "available_date", "report_date", "revenue_qoq_sq"]
    ].copy()
    merged = pd.merge_asof(
        left, right,
        left_on="signal_date", right_on="available_date",
    ...
    return merged.dropna(subset=["revenue_qoq_sq"]).copy()
...
def process_cross_section(signal_factor: pd.DataFrame, group_num: int) -> pd.DataFrame:
    ...
    df = signal_factor.dropna(subset=["revenue_qoq_sq"]).copy()
    g = df.groupby("signal_date")["revenue_qoq_sq"]
    mean = g.transform("mean")
    std = g.transform(lambda s: s.std(ddof=0))
    lower, upper = mean - 3.0 * std, mean + 3.0 * std
    df = df.loc[(df["revenue_qoq_sq"] >= lower) & (df["revenue_qoq_sq"] <= upper)].copy()

    g2 = df.groupby("signal_date")["revenue_qoq_sq"]
    mean2 = g2.transform("mean")
    std2 = g2.transform(lambda s: s.std(ddof=0))
    df["zscore"] = np.where(std2.gt(0), (df["revenue_qoq_sq"] - mean2) / std2, np.nan)
...
def backtest_variant(factor: pd.DataFrame, universe: pd.DataFrame, fwd: pd.DataFrame,
    ...
    """给定单季度营业收入环比增长率因子，做周频 10 组多空 + RankIC + 绩效，落盘到 out_dir。
    ...
    factor.to_csv(out_dir / "01_revenue_qoq_weekly_factor.csv", index=False, encoding="utf-8-sig")
    graded[["signal_date", "stock_code", "revenue_qoq_sq", "roe_ttm", "zscore", "rank_desc",
...
def run(start_date: str, end_date: str, group_num: int, return_column: str,
    ...
    factor = prepare_revenue_qoq_single_quarter(raw_fin, availability=availability)
    ...
    print("\n================= 单季度营业收入环比增长率因子(Revenue_QoQ)周频回测结果摘要 =================")
...
def parse_args():
    p = argparse.ArgumentParser(description="单季度营业收入环比增长率因子(Revenue_QoQ)周频回测（文档口径）")
