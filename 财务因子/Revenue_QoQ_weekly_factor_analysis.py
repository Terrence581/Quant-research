# -*- coding: utf-8 -*-
"""
单季度营收环比增长率因子(RevenueQoQ)复现——图表脚本。
读取 Revenue_QoQ_weekly_factor.py 的输出，绘制对应研报"原始因子"的图：
- 多空组合净值（对应图18左 / 图26左的"原始因子"曲线）
- 分组净值曲线 group_1..group_10（对应图19左）
- 周频 Rank IC 柱状 + 累计 Rank IC（沿用图20 原始因子的展示形式）

运行：  D:/Anaconda/python.exe 财务因子/Revenue_QoQ_weekly_factor_analysis.py
"""
...
from Revenue_QoQ_weekly_factor import (  # noqa: E402
...
def validate_qoq_growth_output(vdir: Path) -> None:
    ...
    factor_path = vdir / "01_revenue_qoq_weekly_factor.csv"
    if not factor_path.exists():
        raise SystemExit(f"未找到 {factor_path.name}，请先运行主脚本生成环比增长率结果。")
    columns = ["revenue_sq", "prev_quarter_sq", "revenue_qoq_sq"]
    factor = pd.read_csv(factor_path, usecols=columns)
    current = pd.to_numeric(factor["revenue_sq"], errors="coerce")
    previous = pd.to_numeric(factor["prev_quarter_sq"], errors="coerce")
    stored = pd.to_numeric(factor["revenue_qoq_sq"], errors="coerce")
    ...
    if bool(mismatch.any()):
        raise SystemExit(
            ...
            "（本季−上季）/|上季|；请重新运行 Revenue_QoQ_weekly_factor.py。"
        )
...
def plot_long_short_nav(ls: pd.DataFrame, path: Path) -> None:
    ...
    ax.plot(ls["signal_date"], ls["long_short_nav"], color="#c0392b", lw=1.6,
            label="原始因子 多空净值")
    ax.set_title(f"单季度营收环比增长率因子(RevenueQoQ) 原始因子 多空组合净值（周频）{FILTER_TAG}")
    ax.set_ylabel("净值")
...
def plot_group_nav(groups: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"RevenueQoQ 原始因子 分组净值{FILTER_TAG}（group_1=因子最高, group_10=因子最低）")
    ax.set_ylabel("净值")
...
def plot_rank_ic(ic: pd.DataFrame, path: Path) -> None:
    ...
    ax1.set_title(f"RevenueQoQ 原始因子 周频 Rank IC（mean {mean_ic*100:.2f}%）{FILTER_TAG}")
    ax1.grid(alpha=0.3)
...
def plot_quantile_sort_bar(groups: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"单季度营收环比增长率排序检验{FILTER_TAG}")
    ax.set_xlabel("单季度营收环比增长率因子分组")
...
def plot_sort_test_table(groups: pd.DataFrame, ls: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title(f"单季度营收环比增长率排序检验结果{FILTER_TAG}", pad=20)
    fig.tight_layout()
    fig.savefig(path, dpi=150)
    plt.close(fig)
...
def plot_high_low_nav(ls: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle(f"单季度营收环比增长率因子 High-Low 周频复利净值走势{FILTER_TAG}", y=0.985, fontsize=13)
...
def plot_ic(ic: pd.DataFrame, path: Path) -> None:
    ...
    ax1.set_title(f"单季度营收环比增长率因子 IC{FILTER_TAG}")
    ax1.set_xlabel("时间")
    ax1.grid(alpha=0.3)
    ax1.xaxis.set_major_locator(mdates.YearLocator())
    ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y"))
