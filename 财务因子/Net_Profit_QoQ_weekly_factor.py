# -*- coding: utf-8 -*-
"""
单季度净利润环比增长率(Net Profit QoQ, single quarter)因子：周频 Rank IC 回测。

参照 财务因子/Sales_YoY_Q1_factor.py 的框架搭建，将标的指标替换为
净利润（financial_data.metric_code = B002000000），时序规则复用本目录
...
运行：  D:/Anaconda/python.exe 财务因子/Net_Profit_QoQ_weekly_factor.py
"""
...
OUTPUT_DIR = PROJECT_DIR / "net_profit_qoq_weekly_output"
...
def load_financial_net_profit(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    """读取净利润长表；多取两年历史，保证起始期 Q1 能拆出上年 Q4 单季值。"""
...
def load_roe_ttm(start_date: str, end_date: str, refresh: bool = False) -> pd.DataFrame:
    ...
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    cache = CACHE_DIR / f"net_profit_qoq_roe_ttm_{ROE_TTM_FIELD}_{start_date}_{end_date}.parquet"
    if cache.exists() and not refresh:
        return pd.read_parquet(cache)
...
def prepare_net_profit_qoq_single_quarter(
    raw_financial: pd.DataFrame,
    availability: str = "deadline",
) -> pd.DataFrame:
    """从 financial_data 计算单季度净利润环比增长率。
    ...
    prev_quarter = fin[["stock_code", "report_date", "net_profit_sq"]].rename(
        columns={"report_date": "prev_q_end", "net_profit_sq": "prev_quarter_sq"}
    )
    ...
    fin["net_profit_qoq_sq"] = np.nan
    fin.loc[valid, "net_profit_qoq_sq"] = (
        (fin.loc[valid, "net_profit_sq"] - fin.loc[valid, "prev_quarter_sq"])
        / fin.loc[valid, "prev_quarter_sq"].abs()
    )
    factor = fin.loc[valid, [
        "stock_code", "report_date", "available_date",
        "net_profit_cum", "net_profit_sq", "prev_quarter_sq", "net_profit_qoq_sq",
    ]].copy()
...
def attach_factor_pit(universe_at_signal: pd.DataFrame, factor: pd.DataFrame) -> pd.DataFrame:
    ...
    right = factor.sort_values(["available_date", "report_date"])[
        ["stock_code", "available_date", "report_date", "net_profit_qoq_sq"]
    ].copy()
    merged = pd.merge_asof(
        left, right,
        left_on="signal_date", right_on="available_date",
    ...
    return merged.dropna(subset=["net_profit_qoq_sq"]).copy()
...
def process_cross_section(signal_factor: pd.DataFrame, group_num: int) -> pd.DataFrame:
    ...
    df = signal_factor.dropna(subset=["net_profit_qoq_sq"]).copy()
    g = df.groupby("signal_date")["net_profit_qoq_sq"]
    mean = g.transform("mean")
    std = g.transform(lambda s: s.std(ddof=0))
    lower, upper = mean - 3.0 * std, mean + 3.0 * std
    df = df.loc[(df["net_profit_qoq_sq"] >= lower) & (df["net_profit_qoq_sq"] <= upper)].copy()

    g2 = df.groupby("signal_date")["net_profit_qoq_sq"]
    mean2 = g2.transform("mean")
    std2 = g2.transform(lambda s: s.std(ddof=0))
    df["zscore"] = np.where(std2.gt(0), (df["net_profit_qoq_sq"] - mean2) / std2, np.nan)
...
def backtest_variant(factor: pd.DataFrame, universe: pd.DataFrame, fwd: pd.DataFrame,
    ...
    """给定单季度净利润环比增长率因子，做周频 10 组多空 + RankIC + 绩效，落盘到 out_dir。
    ...
    factor.to_csv(out_dir / "01_net_profit_qoq_weekly_factor.csv", index=False, encoding="utf-8-sig")
    graded[["signal_date", "stock_code", "net_profit_qoq_sq", "roe_ttm", "zscore", "rank_desc",
...
def run(start_date: str, end_date: str, group_num: int, return_column: str,
    ...
    factor = prepare_net_profit_qoq_single_quarter(raw_fin, availability=availability)
    ...
    print("\n================= 单季度净利润环比增长率因子(Net_Profit_QoQ)周频回测结果摘要 =================")
...
def parse_args():
    p = argparse.ArgumentParser(description="单季度净利润环比增长率因子(Net_Profit_QoQ)周频回测（文档口径）")
