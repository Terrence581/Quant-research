...
如果需要改参数，可以直接修改脚本顶部的 `DEFAULT_` 配置，或使用脚本支持的命令行参数。
...
```text
monthly_lb3m_s0_hd1m_g10_mkt1-4-16-32-64
```
...
IC 用于衡量因子值与未来收益之间的截面相关性：

```text
IC_t = corr(factor_i,t, future_return_i,t)
```

RankIC 使用 Spearman 秩相关：

```text
RankIC_t = spearman_corr(factor_i,t, future_return_i,t)
```
...
```text
calculate_raw_momentum
_calculate_raw_momentum_daily
_calculate_raw_momentum_monthly
```
...
trade_date
stock_code
raw_factor 或 raw_momentum
is_valid_factor 或 is_valid_momentum
missing_reason
```

后续的缩尾、标准化、排序、分组、收益和图表模块可以继续复用。
...
- 月频动量因子研究。
- 月频 `skip` 参数。
- 基于 `return_without_dividend` 的收益率累乘。
